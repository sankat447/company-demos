...
| **4** Load & duration | **Headcount ≠ capacity** — demand (provider-minutes/weekday) from the **forecast model served by KServe** ÷ staffed minutes; flags the over-loaded day + a dynamic "move one provider" rebalance | `load_balance` calls `models.demand_forecast()` → KServe `nychhc-forecast`; chat + Planning |
