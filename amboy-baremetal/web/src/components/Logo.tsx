// Amboy Bank brand mark — recreated as inline SVG (blue triangle + three white
// waves) so it ships self-contained (no external image / CSP concerns).
const BLUE = "#1B9DD9";

export function LogoMark({ size = 30 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 100 100" role="img" aria-label="Amboy Bank">
...
// Full lockup: mark + "AMBOY" (heavy sans) + "Bank" (serif).
export function Logo({ variant = "dark", size = 30 }: { variant?: "dark" | "light"; size?: number }) {
  const amboy = variant === "dark" ? "#FFFFFF" : "#14193D";
  const bank = variant === "dark" ? "#CBD5E1" : "#5A6B86";
  return (
    <span className="inline-flex items-center gap-2 select-none">
      <LogoMark size={size} />
      <span className="leading-none">
        <span className="font-body font-extrabold tracking-tight" style={{ color: amboy, fontSize: size * 0.66 }}>
          AMBOY
        </span>{" "}
